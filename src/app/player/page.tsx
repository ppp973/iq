...
import { X, Loader2, ExternalLink, RefreshCw, TriangleAlert } from 'lucide-react';
...
function PlayerContent() {
  ...
  // Tera Proxy URL (Make sure ye sahi ho)
  const PROXY_PLAYER_BASE = "https://lalitproxy.vercel.app/player?url=";

  useEffect(() => {
    const rawUrl = searchParams.get('url');
    if (rawUrl) {
      // Spaces hatana zaroori hai
      setVideoUrl(rawUrl.trim());
    }
  }, [searchParams]);

  // Agar 5 second tak load na ho to Error dikhaye
  useEffect(() => {
    const timer = setTimeout(() => {
        // Sirf user ko option dikhane ke liye, video band nahi karenge
  ...
  if (!videoUrl) {
    return (
      <div className="flex flex-col items-center justify-center h-[100dvh] bg-black text-white">
        ...
        <p className="text-xs font-mono tracking-widest text-zinc-500">GETTING LINK...</p>
      </div>
    );
  }

  // Final Proxy URL Generation
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-black overflow-hidden flex flex-col">
      ...
      <div className="absolute top-0 left-0 w-full p-4 z-50 flex justify-between items-start pointer-events-none bg-gradient-to-b from-black/90 to-transparent h-24">
        ...
        >
           <span className="text-[10px] font-bold text-white tracking-wider">OPEN EXTERNALLY</span>
      ...
      <div className="w-full h-full relative z-10 bg-black">
        <iframe 
          ...
          // Referrer Policy hata di hai taaki Proxy sahi se kaam kare
          style={{ width: '100%', height: '100%', backgroundColor: '#000' }}
        ></iframe>
      </div>

      {/* --- ERROR FALLBACK (Agar video atak jaye) --- */}
      {isError && (
        <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-40 bg-[#111] border border-red-500/30 p-4 rounded-2xl w-[90%] max-w-sm flex items-center justify-between shadow-2xl">
           <div className="flex items-center gap-3">
             ...
             <div>
               <p className="text-xs font-bold text-white">Video not playing?</p>
               <p className="text-[10px] text-zinc-400">Try opening in native player</p>
             </div>
           </div>
           <a href={finalSrc} target="_blank" className="bg-white text-black text-xs font-bold px-3 py-2 rounded-lg">
             Open
           </a>
           <button onClick={() => setIsError(false)} className="absolute -top-2 -right-2 bg-white rounded-full p-1 text-black"><X size={12}/></button>
        </div>
      )}
...
export default function VideoPlayerPage() {
  return (
    <div className="fixed inset-0 z-[9999] bg-black h-[100dvh] w-screen">
      <Suspense fallback={<div className="h-full w-full flex items-center justify-center bg-black"><Loader2 className="animate-spin text-indigo-500"/></div>}>
        <PlayerContent />
      </Suspense>
    </div>
  );
